...
**https://blockdmask.tistory.com/22 참고**
...
- 프로세스는 환전한 독립된 두 개의 프로그램 실행을 위해서 사용되는 반면 쓰레드는 하나의 프로그램 내에서 둘 이상의 프로그램 흐름을 만들어 내기 위해서 디자인 됨
- 쓰레드에서는 프로세스와 다르게 공유하는 상태정보들이 있음(쓰레드의 컨텍스트 스위칭을 빠르게 하는 요인)
- 하나의 프로세스의 여러 쓰레드는 프로세스의 **Code, Data, Heap**(Stack 제외)를 공유한다.
...
- 스케줄러란 어떤 프로세스에게 자원을 할당할 지를 결정하는 운영체제 커널의 모듈을 지칭한다.
...
- 장기 스케줄러: 어떤 프로세스를 준비큐에 넣을 것인가?

작업 스케줄러라고도 부르며 어떤 프로세스를 준비 큐에 삽입할지를 결정한다.
...
cpu 스케줄러라고도 하며 준비 상태의 프로세스 중에서 어떤 프로세스를 다음 번에 실행 상태로 만들 것인지를 결정한다. 시분할 시스템에서 타이머 인트럽트가 발생하면 단기 스케줄러가 호출된다. 일반적으로 스케줄러라 함은 단기 스케줄러를 의미한다. 
...
너무 많은 프로세스에게 메모리를 할당해 시스템의 성능이 저하되는 경우 이를 해결하기 위해 메모리에 적재된 프로세스의 수를 동적으로 조절하기 위해 추가된 스케줄러이다. 만약 메모리에 많은 수의 프로세스가 적재되어 프로세스 당 보유하고 있는 메모리량이 극도로 적어지게 되면 CPU 수행에 당장 필요한 프로세스의 주소 공간조차도 메모리에 올려놓기 어려운 상황이 발생한다. 그렇게 되면 디스크 I/O가 수시로 발생하게 되어 시스템의 성능이 심각하게 저하될 수 있다. 이런 경우 메모리에 올라와 있는 프로세스 중 일부로부터 메모리를 통째로 빼앗아 그 내용을 디스크의 스왑 영역에 저장해 둔다. 이와 같은 행위를 스왑 아웃 이라고 한다. 
...
**참고 링크 https://kosaf04pyh.tistory.com/191**
...
- 선점형 스케줄링 : 프로세스가 CPU를 할당받아 실행 중이더라도 운영체제가 CPU를 강제로 빼앗을 수 있는 방식, CPU 처리 시간이 매우 긴 프로세스가 CPU 사용 독점을 막을 수 있어 효율적인 운영이 가능하다. 하지만 잦은 문맥 교환으로 오버헤드가 많이 발생한다.

- 비선점형 스케줄링 : 프로세스가 CPU를 점유하고 있다면 이를 빼앗을 수 없는 방식 필요한 문맥 교환만 일어나기 때문에 오버헤드가 상대적으로 적지만 프로세스의 배치에 따라 효율성 차이가 많이 난다.

_스케줄링 대상은 Ready Queue 에 있는 프로세스들이다._

**참고 링크 https://hyunah030.tistory.com/4**
...
즉, RAM에서 메모리의 공간이 작은 조각으로 나뉘어져 사용가능한 메모리가 충분히 존재하지만 할당(사용)이 불가능한 상태를 보고 메모리 단편화가 발생했다고 한다.
...
즉, 외부 단편화는 메모리가 할당되고 해제되는 작업이 반복되면서 작은 공간의 메모리 영역이 생기게 되는데 이 공간의 크기가 어떤 프로세스가 필요로 하는 크기보다 작은 경우를 의미한다. 쉽게 말해서 충분한 공간이 있지만 내가  필요로 하는 크기보다 작아서 사용할 수 없는 경우이다. 


* 내부 단편화: 프로세스가 사용하는 메모리 공간 에 포함된 남는 부분.
즉, 내부 단편화는 프로세스에 할당된 메모리 공간이 실제로 프로세스가 필요한 공간보다 많이 할당되었을 때, 프로세스 내부에서 발생되는 단편화를 말한다.

**참고 링크: https://kosaf04pyh.tistory.com/40**
...
**참고 링크: https://jhnyang.tistory.com/290**
...
- 페이지 폴트란 프로그램이 자신의 주소 공간(가상메모리 공간)에는 존재하지만 시스템의 RAM에는 현재 없는 데이터나 코드에 접근 시도하였을 경우 발생하는 현상이다.
